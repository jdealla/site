...
export function getPlayerData(id) {
    ...
    const finishingBadges = getPlayerInfo(id, playerEnum.BADGES.FINISHING.START, playerEnum.BADGES.FINISHING.END);
    const shootingBadges = getPlayerInfo(id, playerEnum.BADGES.SHOOTING.START, playerEnum.BADGES.SHOOTING.END);
    const playmakingBadges = getPlayerInfo(id, playerEnum.BADGES.PLAYMAKING.START, playerEnum.BADGES.PLAYMAKING.END);
    const defensiveBadges = getPlayerInfo(id, playerEnum.BADGES.DEFENSE.START, playerEnum.BADGES.DEFENSE.END)
    const personalityBadges = getPlayerInfo(id, playerEnum.BADGES.PERSONALITY.START, playerEnum.BADGES.PERSONALITY.END);
...
export function getPropNames(type) {
    ...
    switch(type) {
        case "stats": 
            return allKeys.slice(playerEnum.STATS.STARTINDEX, playerEnum.STATS.ENDINDEX).map(key => formatName(key));
        case "tendencies":
            return allKeys.slice(playerEnum.TENDENCIES.STARTINDEX, playerEnum.TENDENCIES.ENDINDEX).map(key => formatName(key));
        case "badges":
            return allKeys.slice(playerEnum.BADGES.STARTINDEX, playerEnum.BADGES.ENDINDEX).map(key => formatName(key));
        case "sigs":
            return allKeys.slice(playerEnum.SIGS.STARTINDEX, playerEnum.SIGS.ENDINDEX).map(key => formatName(key));
    }
}
