...
export async function getPlayerData(id) {
    ...
    const player = await session.query({ collection: "Players" })
                        .whereEquals("pid", id)
                        .first();
    ...
    const playerInfo = {
        pid: player.pid, name: player.name, college: player.college, nickname: player.nickname, collection: player.collection, theme: player.theme, 
