...
export async function getAllPlayersWithAllStats() {
    const players = []
    const session = store.openSession();
    const res = await session.query({ indexName: "Players/BasicInfo" })
                                .orderByDescending("overall")
                                .take(1024)
                                .all();

    const res2 = await session.query({ indexName: "Players/BasicInfo" })
                                .orderByDescending("overall")
                                .skip(1024)
                                .all();
    ...
    res.forEach(player => players.push(formatPlayerObject(player)));
    res2.forEach(player => players.push(formatPlayerObject(player)));
    return players;
}
...
export async function findDuos(id) {
    const session = store.openSession();
    const res = await session.query({ collection: "Duos" }).whereEquals("name", name).all();
