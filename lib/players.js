...
import { formatName, levelToNum } from "./helpers";
...
export function getPlayersInfoData() {
    return allPlayers.map(player => getPlayerData(player.id).info)
}
...
export function getPlayerBySuggestion(value) {
    ...
    return inputLength === 0 ? [] : allPlayers.map(player => getPlayerData(player.id).info).filter(player => {
...
export function filterPlayersByProp(prop, value) {
    return allPlayers.filter(player => player[prop] > value)
}

export function groupedPlayersByProp(prop) {
    let groupedBy = allPlayers.reduce((h, obj) => Object.assign(h, { [obj[prop]]: ( h[obj[prop]] || [] ).concat(obj) }), {});
...
export function getPlayersByPropValue(prop, value) {
    return groupedPlayersByProp(prop)[value];
}
...
export function getUpdateDates() {
    return Object.keys(groupedPlayersByProp("date")).map(date => {
        return {
            params: {
                date: date
            }
...
export function getCollections() {
    return Object.keys(groupedPlayersByProp("collection")).map(name => {
        return {
            params: {
...
export function getThemes() {
    const grouped = groupedPlayersByProp("theme");

    return Object.entries(grouped).slice(0, 2).map(([name, value]) => {
        return {
            params: {
                name: value[0].collection.toLowerCase().replace(/ /g, "-"),
                themeName: name.toLowerCase().replace(/ /g, "-")
            }
        }
    }) 
}
