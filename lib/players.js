...
export async function getAllProps() {
    const session = store.openSession();
    const player = await session.query({ collection: "Players" }).first();
    
    await session.saveChanges();

    const allProps = Object.keys(player);

    const info = allProps.slice(0, 13);

    const stats = allProps.slice(31, 69);

    const tendencies = allProps.slice(69, 158);

    const badges = allProps.slice(158, 246);
    
    const animations = allProps.slice(246, 284);
